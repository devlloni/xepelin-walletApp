...
const WithdrawalPage = () => {
    ...
    const [ currentBalance, setCurrentBalance ] = useState<null|number>(null);
    ...
    const realiceTransfer = async () => {
        if(currentBalance && Number(amount) > currentBalance) {
            return toast('¡No tienes suficiente dinero en tu cuenta!', { type: 'error' });
        }
        else {
            console.log('entra aca', {amount, prueba: Number(amount)})
            if ((Number(amount) && Number(amount) > 0) && session?.user?.accountNumber) {
                const withdrawalBody = {
                    from: session?.user?.accountNumber,
                    to: 0,
                    amount: Number(amount),
                    transactionType: 'withdrawal'
                }
                const resp = await axios.post(`${API_BASE_URL}/transactions`, withdrawalBody);
                if (resp.status === 201 && resp.data) {
                    console.log('Retiro realizado.', resp.data);
                    toast('¡Retiro de dinero realizado!', { type: 'success' });
                    if(currentBalance) {
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <p className='font-medium text-sm'>Balance actual: {currentBalance ? currentBalance : 'Cargando..'}</p>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label htmlFor="amount" className="block text-sm font-medium text-gray-700">Cantidad a retirar</label>
                <div className="mt-1">
                  <input
                    id="amount"
                    name="amount"
              ...
              <div>
                <button disabled={buttonDisabled} type="submit" className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                  Retirar dinero
                </button>
              </div>
            </form>
